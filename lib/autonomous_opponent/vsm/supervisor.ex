defmodule AutonomousOpponent.VSM.Supervisor do
  ...
  def init(_opts) do
    ...
    opts = [
      strategy: :one_for_one,  # Subsystems are independent
      max_restarts: 5,
      max_seconds: 60
    ]
  ...
  def health_check do
    ...
    health_results = Enum.map(subsystems, fn subsystem ->
      pid = get_subsystem_pid(subsystem)
      
      health = if pid && Process.alive?(pid) do
    ...
    overall_health = if Enum.all?(Map.values(health_results), &(&1 == :healthy)) do
      :healthy
    else
      :degraded
    end
