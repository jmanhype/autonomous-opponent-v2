defmodule AutonomousOpponent.VSM.S5.GovernanceEngine do
  ...
  def handle_call({:make_decision, issue, context}, _from, state) do
    ...
    decision = if should_create_policy?(analysis) do
  ...
  defp init_policy_templates do
    %{
      resource_allocation: %{
        ...
        review_interval: 300_000,  # 5 minutes
        directives: [:optimize, :balance, :prioritize]
      },
      emergency_response: %{
        type: :crisis_management,
        duration: 1800_000,  # 30 minutes
        review_interval: 60_000,  # 1 minute
        directives: [:mitigate, :isolate, :recover]
      },
      adaptation: %{
        type: :system_evolution,
        duration: nil,  # Permanent
        review_interval: 3600_000,  # 1 hour
  ...
  defp perform_action_evaluation(action, context, state) do
    ...
    # Determine decision
    decision = if weighted_score >= state.evaluation_state.threshold do
      :approved
    else
      :rejected
    end
  ...
  defp evaluate_goal_contribution(action, goals) do
    ...
    contributing_goals = Enum.filter(goals, fn goal ->
    ...
    if Enum.empty?(contributing_goals) do
      ...
      total_priority = contributing_goals
                      |> Enum.map(& &1.priority)
                      |> Enum.sum()
  ...
  defp evaluate_constitutional_compliance(action, invariants) do
    ...
    violations = Enum.filter(invariants, fn {invariant, _desc} ->
  ...
  defp calculate_trait_alignment(action, traits) do
    ...
    if Enum.empty?(relevant_traits) do
      ...
      trait_scores = Enum.map(relevant_traits, fn trait ->
