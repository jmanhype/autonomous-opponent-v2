defmodule AutonomousOpponent.VSM.S2.Coordination do
  ...
  @coordination_interval 2_000  # 2 seconds - matches S1 rhythms
  @oscillation_threshold 0.3    # 30% variation - Beer's empirical threshold
  @damping_factor 0.7          # Critical damping approximation
  ...
  def handle_call({:apply_damping, units, params}, _from, state) do
    result = DampingController.apply_damping(
  ...
  def handle_info(:coordination_cycle, state) do
    ...
    new_state = if not Enum.empty?(oscillations) do
  ...
  defp init_coordination_rules do
    %{
      ...
      min_coordination_interval: 1000,  # 1 second
  ...
  defp calculate_damping_parameters(oscillation) do
    %{
      ...
      duration: 10_000  # 10 seconds = 5 coordination cycles, allows gradual correction
    }
  end
  ...
  defp resolve_resource_contention(contention_data, state) do
    ...
    resolution = case state.coordination_rules.contention_resolution do
  ...
  defp release_lock_and_process_queue(unit_id, resource, state) do
    ...
    case Map.get(state.resource_locks, lock_key) do
      ...
      _ ->
        state  # Not locked by this unit
    end
  end
  
  defp process_lock_queue(resource, state) do
    # Find pending requests for this resource
    {matching, remaining} = Enum.split_with(state.coordination_state.pending, fn req ->
  ...
  defp coordinate_new_unit(unit_id, state) do
    ...
    other_units = state.s1_units
    |> Map.keys()
    |> Enum.filter(&(&1 != unit_id))
    |> Enum.take(2)  # Coordinate with up to 2 other units
  ...
  defp handle_lock_timeout(lock_id, state) do
    ...
    expired_lock = state.resource_locks
  ...
  defp resolve_by_performance(contention, _state), do: %{winner: hd(contention.units), strategy: :performance}
