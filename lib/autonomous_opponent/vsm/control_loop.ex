defmodule AutonomousOpponent.VSM.ControlLoop do
  ...
  @loop_interval 1000  # 1 second control loop
  ...
  @health_check_interval 5000  # 5 seconds health check
  ...
  def handle_info(:control_loop, state) do
    ...
    {_result, new_state} = if state.emergency_mode do
      execute_emergency_cycle(state)  # Survival mode - speed over completeness
    else
      execute_control_cycle(state)     # Normal mode - full integration
    end
  ...
  defp init_control_state do
    %{
      cycle_count: 0,
      last_cycle: nil,
      flow_direction: :forward,  # Always forward - prevents chaos
      active_channels: MapSet.new([:s1_s2, :s2_s3, :s3_s4, :s4_s5, :s5_s1]),
      blocked_channels: MapSet.new()  # Failed channels get blocked, not fixed
    }
  end
  ...
  defp execute_emergency_cycle(state) do
    ...
    try do
      ...
      s5_directives = govern_through_s5(%{decisions: s3_decisions, emergency: true}, state.subsystems.s5)
  ...
  defp check_subsystem_health(subsystems) do
    subsystem_health = Enum.map(subsystems, fn {name, pid} ->
      health = if is_pid(pid) and Process.alive?(pid) do
    ...
    overall = if Enum.all?(Map.values(subsystem_health), &(&1 == :operational)) do
      :healthy
    else
      :degraded
    end
  ...
  defp handle_health_crisis(health_status, state) do
    ...
    failed_count = Enum.count(health_status.subsystems, fn {_name, status} ->
      status == :failed
    end)
  ...
  defp handle_subsystem_failure(failure, state) do
    ...
    new_control_state = if failed_channel do
      %{state.control_state |
  ...
  defp update_cycle_metrics(metrics, result, cycle_time) do
    case result do
      :success ->
        ...
        avg_time = if metrics.average_cycle_time == 0 do
          cycle_time
        else
  ...
  defp identify_failed_channel(failure) do
    case failure[:subsystem] do
      :s1 -> :s1_s2    # S1 can't send operational data
      :s2 -> :s2_s3    # S2 can't coordinate
      :s3 -> :s3_s4    # S3 can't optimize
      :s4 -> :s4_s5    # S4 can't provide intelligence
      :s5 -> :s5_s1    # S5 can't send directives
      _ -> nil         # Unknown subsystem - no channel to block
    end
  end
end