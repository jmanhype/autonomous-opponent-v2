defmodule AutonomousOpponent.VSM.S4.Intelligence do
  ...
  def handle_call({:scan_environment, focus_areas}, _from, state) do
    case perform_environmental_scan(focus_areas, state) do
      {:ok, scan_results, new_state} ->
        ...
        updated_model = update_environmental_model(
  ...
  def handle_call({:extract_patterns, data_source}, _from, state) do
    ...
    new_library = update_pattern_library(
      state.pattern_library,
  ...
  def handle_call({:model_scenarios, params}, _from, state) do
    ...
    scenarios_with_uncertainty = Enum.map(scenarios, fn scenario ->
      Map.put(scenario, :uncertainty, calculate_uncertainty(scenario, state))
    end)
    
    # Update projections
    new_projections = Map.put(
  ...
  defp init_environmental_model do
    %{
      ...
      spatial_patterns: [],   # Patterns in space/structure
      causal_chains: [],      # What causes what
      last_update: System.monotonic_time(:millisecond),
      version: 1              # Model evolution tracking
    }
  end
  ...
  defp init_pattern_library do
    %{
      operational_patterns: %{},    # How the system behaves
      environmental_patterns: %{},  # How the world behaves
      behavioral_patterns: %{},     # How users/actors behave
      anomaly_patterns: %{},        # What doesn't fit (often most valuable!)
      cross_domain_patterns: %{}    # Patterns that span categories
    }
  end
  ...
  defp init_belief_integration do
    %{
      ...
      synchronization_interval: 30_000,         # 30s - consciousness breathing rate
      conflict_resolution: :last_write_wins,    # Simple but effective
      belief_confidence_threshold: 0.6          # 60% - balanced skepticism
    }
  end
  ...
  defp init_llm_connection do
    %{
      ...
      amplification_factor: 10,        # 10x pattern detection capability
      query_templates: load_query_templates(),
      rate_limit: 100                  # queries per minute - cost control
    }
  end
  ...
  defp perform_environmental_scan(focus_areas, state) do
    try do
      ...
      scan_data = EnvironmentalScanner.scan(
      ...
      amplified_data = if llm_available?(state) do
  ...
  defp amplify_scan_with_llm(scan_data, state) do
    ...
    case query_llm(state.llm_connection, scan_data) do
      ...
      {:error, _reason} ->
        scan_data  # Fall back to unamplified data - resilience over features
    end
  end
  ...
  defp update_environmental_model(model, scan_results) do
    ...
    new_relationships = merge_relationships(
      model.relationships,
    ...
    %{model |
      ...
      last_update: System.monotonic_time(:millisecond),
      version: model.version + 1    # Track model evolution
    }
  end
  ...
  defp extract_operational_patterns(buffer) do
    ...
    Enum.flat_map(grouped, fn {source, events} ->
      case source do
        ...
        _ -> []  # Unknown sources ignored for now
      end
    end)
  end
  ...
  defp calculate_uncertainty(scenario, state) do
    ...
    base_uncertainty = 0.1                    # 10% - chaos theory minimum
  ...
  defp calculate_model_confidence(model) do
    ...
    entity_score = min(1.0, map_size(model.entities) / 100)      # 100 entities = mature
    relationship_score = min(1.0, length(model.relationships) / 500)  # 500 connections = rich
    pattern_score = min(1.0, length(model.temporal_patterns) / 50)    # 50 patterns = experienced
  ...
  defp significant_belief_change?(updates) do
    ...
    length(updates) > 5 or    # Multiple beliefs changing = paradigm shift
    Enum.any?(updates, fn update -> update[:priority] == :critical end)  # Critical = immediate
  end
  ...
  defp categorize_pattern(pattern) do
    case pattern.type do
      :variety_absorption -> :operational_patterns   # Internal operations
      :coordination -> :operational_patterns        # Internal operations
      :environmental -> :environmental_patterns     # External world
      :behavioral -> :behavioral_patterns           # Actor behaviors
      :anomaly -> :anomaly_patterns                 # Rule breakers (treasure these!)
      _ -> :cross_domain_patterns                   # Patterns that transcend categories
    end
  end
  ...
  defp merge_relationships(existing, new) do
    ...
    |> Enum.take(1000)  # Limit size - quality over quantity
  end
  ...
  defp detect_temporal_patterns(existing_patterns, scan_results) do
    ...
    (existing_patterns ++ new_patterns) |> Enum.take(100)  # Keep strongest patterns
  end
  ...
  defp calculate_trend(values) do
    ...
    if length(values) < 2 do
      :stable  # Can't trend with one point
    else
