defmodule AutonomousOpponent.VSM.S4.VectorStore.HNSWIndex do
    ...
    final_state = if opts[:persist_path] do
      ...
      if Persistence.index_exists?(opts[:persist_path]) do
        case Persistence.load_index(opts[:persist_path]) do
          ...
          {:error, reason} ->
            Logger.warn("Failed to restore index: #{inspect(reason)}, starting fresh")
            state
        end
      else
        state
  ...
  defp insert_node(state, node_id, vector, level) do
    ...
    for lc <- level..0 do
      ...
      candidates = search_layer_for_insertion(state, vector, entry_points, state.ef_construction, lc)
      ...
      entry_points = Enum.map(m_nearest, fn {_, id} -> id end)
    end
  end
