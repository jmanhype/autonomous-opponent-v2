defmodule AutonomousOpponent.VSM.S3.Control do
  ...
  @bargaining_interval 5_000  # 5 seconds
  ...
  @prediction_horizon 60_000  # 1 minute ahead
  ...
  def handle_info(:bargaining_round, state) do
    ...
    new_state = if should_run_bargaining?(state) do
  ...
  def handle_info(:update_predictions, state) do
    ...
    # too slow = lag.
    new_filters = Enum.map(state.kalman_filters, fn {resource, filter} ->
  ...
  defp allocate_resources(unit_id, request, state) do
    ...
    if resources_available?(request, state.resource_pool) do
      ...
      allocation = %{
        ...
        expires: System.monotonic_time(:millisecond) + 300_000  # 5 min lease
      }
  ...
  defp return_resources_to_pool(unit_id, resources, state) do
    case Map.get(state.allocations, unit_id) do
      ...
      allocation ->
        ...
        new_pool = Enum.reduce(resources, state.resource_pool, fn {resource, amount}, acc ->
          update_in(acc, [resource, :available], &(&1 + amount))
          |> update_in([resource, :reserved], &(&1 - amount))
        end)
  ...
  defp maybe_trigger_bargaining(state, unit_id, request) do
    if not state.bargaining_state.active do
  ...
  defp run_bargaining_round(state) do
    ...
    bargaining_result = ResourceBargainer.negotiate(
      participants,
      state.allocations,
      state.resource_pool,
  ...
  defp calculate_total_utilization(pool) do
    total_available = Enum.reduce(pool, 0, fn {_, res}, acc -> 
      acc + res.available 
    end)
    
    total_capacity = Enum.reduce(pool, 0, fn {_, res}, acc -> 
      acc + res.total 
    end)
  ...
  defp resource_imbalance?(state) do
    utilizations = Enum.map(state.resource_pool, fn {_, resource} ->
    ...
    (max_util - min_util) > 0.3  # 30% imbalance threshold
  end
  ...
  defp apply_bargaining_results(results, state) do
    ...
    Enum.reduce(results.reallocations, state, fn {from_unit, to_unit, resources}, acc ->
      ...
      |> elem(2)  # Get the state from {:ok, _, state}
    end)
  end
  ...
  defp calculate_demand_rate(resource, state) do
    ...
    recent_allocations = Enum.filter(state.allocations, fn {_, alloc} ->
      age = System.monotonic_time(:millisecond) - alloc.timestamp
      age < 60_000  # Last minute
    end)
    
    length(recent_allocations) / 60.0  # Allocations per second
  end
