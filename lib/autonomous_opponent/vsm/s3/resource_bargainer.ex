defmodule AutonomousOpponent.VSM.S3.ResourceBargainer do
  ...
  defp collect_bids(state) do
    ...
    bids = Enum.map(state.participants, fn participant ->
      ...
      resource_bids = Enum.map([:cpu, :memory, :variety_capacity], fn resource ->
  ...
  defp collect_offers(state) do
    ...
    offers = Enum.map(state.participants, fn participant ->
      current_alloc = Map.get(state.allocations, participant, %{})
      performance = estimate_performance(participant, current_alloc)
      
      # Generate offers for resources they can spare
      resource_offers = Enum.map([:cpu, :memory, :variety_capacity], fn resource ->
  ...
  defp apply_trades(state) do
    ...
    new_allocations = Enum.reduce(state.matched_trades, state.allocations, fn trade, allocs ->
      allocs
  ...
  defp calculate_offer_amount(resource, performance, allocation) do
    ...
    case resource do
      :cpu ->
        max(0, current * 0.2)  # Offer up to 20% of CPU
      
      :memory ->
        max(0, current * 0.3)  # Offer up to 30% of memory
      
      :variety_capacity ->
        max(0, current * 0.25) # Offer up to 25% of variety capacity
  ...
  defp calculate_urgency(performance, targets) do
    ...
    response_gap = max(0, performance.response_time - targets.response_time) / targets.response_time
  ...
  defp calculate_max_price(resource, performance) do
    ...
    base_price * (1 + calculate_urgency(performance, %{variety_absorption: 0.9, response_time: 100}) / 10)
  end
  ...
  defp calculate_min_price(resource, performance) do
    ...
    base_price * 0.5  # Willing to accept 50% of base price
  end
