defmodule AutonomousOpponent.VSM.S3.KalmanFilter do
  ...
  defstruct [
    ...
    :error_covariance,    # Estimation error covariance
  ...
  def new(resource_type, opts \\ []) do
    %__MODULE__{
      ...
      transition_matrix: 1.0,  # Simple scalar model
      measurement_matrix: 1.0,  # Direct observation
      history: [],
  ...
  def update(filter, measurement) do
    ...
    predicted_covariance = filter.transition_matrix * filter.error_covariance * 
    ...
    innovation_covariance = filter.measurement_matrix * predicted_covariance * 
    ...
    # Add to history
    new_history = [{measurement.timestamp, measurement} | filter.history]
  ...
  def predict(filter, horizon_ms) do
    ...
    steps = horizon_ms / 1000  # Convert to seconds
    
    predicted_state = filter.state_estimate + 
                     (calculate_trend(filter) * steps)
    
    # Increase uncertainty over time
    predicted_uncertainty = filter.error_covariance * 
  ...
  defp default_initial_state do
    0.5  # Start with 50% utilization estimate
  end
  
  defp default_process_noise(resource_type) do
    # Different resources have different volatility
    case resource_type do
      :cpu -> 0.05              # CPU is quite volatile
      :memory -> 0.02           # Memory changes slowly
      :variety_capacity -> 0.1  # Variety is highly variable
      :processing_slots -> 0.03 # Slots change moderately
      _ -> 0.05
    end
  end
      ...
      history ->
        ...
        if length(recent) < 2 do
          ...
          if denominator == 0 do
            ...
            (n * sum_xy - sum_x * sum_y) / denominator / 1000  # Per second
          end
        end
    end
  end
  ...
  defp assess_filter_health(filter) do
    cond do
      ...
      length(filter.history) < 5 ->
        :warming_up  # Not enough data
      
      filter.error_covariance < 0.01 ->
        :overconfident  # Might be overfitting
