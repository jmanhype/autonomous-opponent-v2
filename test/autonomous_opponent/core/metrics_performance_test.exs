defmodule AutonomousOpponent.Core.MetricsPerformanceTest do
  ...
  setup do
    ...
    {:ok, pid} = Metrics.start_link(
      name: metrics_name,
      persist_interval_ms: 300_000  # 5 minutes to avoid I/O during tests
    )
    ...
    test "mixed workload performance", %{metrics_name: metrics} do
      ...
      tasks = for i <- 1..10 do
        Task.async(fn ->
          for j <- 1..1000 do
            case rem(j, 4) do
              0 -> Metrics.counter(metrics, "mixed.requests", 1, %{worker: i})
              1 -> Metrics.gauge(metrics, "mixed.memory", j * 1024, %{worker: i})
              2 -> Metrics.histogram(metrics, "mixed.latency", :rand.uniform() * 100, %{worker: i})
              3 -> Metrics.vsm_metric(metrics, Enum.random([:s1, :s2, :s3, :s4, :s5]), 
                                     "operations", j)
            end
          end
        end)
        ...
        Task.async(fn ->
          for i <- 1..ops_per_process do
            ...
            case :rand.uniform(5) do
              1 -> Metrics.counter(metrics, "stress.counter", 1, %{proc: p})
              2 -> Metrics.gauge(metrics, "stress.gauge", i, %{proc: p})
              3 -> Metrics.histogram(metrics, "stress.histogram", :rand.uniform() * 100)
              4 -> Metrics.variety_flow(metrics, Enum.random([:s1, :s2, :s3, :s4, :s5]), 
                                       :rand.uniform(100), :rand.uniform(80))
              5 -> Metrics.algedonic_signal(metrics, Enum.random([:pain, :pleasure]), 
                                          :rand.uniform(10), "stress_test")
            end
          end
        end)
      end
