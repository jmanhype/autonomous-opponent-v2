defmodule AutonomousOpponentV2Core.WebGateway.Gateway do
  @moduledoc """
  Main supervisor for Web Gateway Transport implementation.
  ...
  def init(_opts) do
    children = [
      {Registry, keys: :unique, name: AutonomousOpponentV2Core.WebGateway.ConnectionRegistry},
      {Registry, keys: :duplicate, name: AutonomousOpponentV2Core.WebGateway.TransportRegistry},
      {Registry, keys: :unique, name: AutonomousOpponentV2Core.WebGateway.ConfigRegistry},
      AutonomousOpponentV2Core.WebGateway.Pool.ConnectionPool,
      AutonomousOpponentV2Core.WebGateway.LoadBalancer.ConsistentHash,
      AutonomousOpponentV2Core.WebGateway.Transport.Router,
      AutonomousOpponentV2Core.WebGateway.ConnectionDrainer,
      {Task.Supervisor, name: AutonomousOpponentV2Core.WebGateway.TaskSupervisor}
    ]
  ...
  defp calculate_transport_error_rate(transport) do
    case AutonomousOpponentV2Core.WebGateway.Transport.Router.get_error_rate(transport) do
      {:ok, rate} -> rate
      _ -> 0.0
    end
  end
  ...
  def graceful_shutdown(opts \\ []) do
    Logger.info("Initiating graceful shutdown of Web Gateway")
    
    # Trigger algedonic signal for shutdown
    trigger_algedonic(:medium, :planned_shutdown)
    
    # Start connection draining
    case AutonomousOpponentV2Core.WebGateway.ConnectionDrainer.start_draining(opts) do
      :ok ->
  ...
  def accepting_connections? do
    AutonomousOpponentV2Core.WebGateway.ConnectionDrainer.accepting_connections?()
  end
  ...
  def force_shutdown do
    Logger.warning("Forcing immediate shutdown of Web Gateway")
    
    # Trigger critical algedonic signal
    trigger_algedonic(:critical, :forced_shutdown)
    
    # Force shutdown
    AutonomousOpponentV2Core.WebGateway.ConnectionDrainer.force_shutdown()
