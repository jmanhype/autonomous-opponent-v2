defmodule AutonomousOpponentV2Core.WebGateway.Transport.HTTPSSE do
  @moduledoc """
  HTTP+SSE (Server-Sent Events) transport implementation for Web Gateway.
  ...
  def init(_opts) do
    ...
    EventBus.subscribe(:web_gateway_sse_events)
  ...
  @impl true
  def handle_info({:event_bus, :web_gateway_sse_events, %{event: event, data: data}}, state) do
    # Forward Web Gateway events to SSE clients
