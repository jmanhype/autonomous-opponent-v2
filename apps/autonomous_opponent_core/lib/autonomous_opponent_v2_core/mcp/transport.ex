...
defmodule AutonomousOpponentV2Core.MCP.Transport.Stdio do
  ...
  Reads from STDIN and writes to STDOUT using line-delimited JSON.
  Each message is a single line containing a JSON-RPC message.
  """
  ...
  def init(_opts) do
    ...
    Task.start_link(fn -> read_stdin_loop() end)
  ...
  def handle_cast({:send_message, message}, state) do
    case Message.serialize(message) do
      ...
      {:error, reason} ->
        Logger.error("Failed to serialize message: #{inspect(reason)}")
    end
  ...
  defp read_stdin_loop do
    case IO.read(:stdio, :line) do
      :eof ->
        Logger.info("STDIN closed, stopping MCP server")
        GenServer.stop(AutonomousOpponentV2Core.MCP.Server)
  ...
  defp handle_stdin_line("") do
    # Skip empty lines
    :ok
  end
  
  defp handle_stdin_line(line) do
