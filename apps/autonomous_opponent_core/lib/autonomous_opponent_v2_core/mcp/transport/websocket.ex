defmodule AutonomousOpponentV2Core.MCP.Transport.WebSocket do
  @moduledoc """
  WebSocket transport implementation for MCP Gateway.
  ...
  def init(_opts) do
    ...
    EventBus.subscribe(:mcp_websocket_events)
  ...
  @impl true
  def handle_info({:event_bus, :mcp_websocket_events, message}, state) do
  ...
  defp process_incoming_message(connection, message, state) do
    ...
    case parsed do
      {:ok, %{type: type, data: data}} ->
        ...
        EventBus.publish(:mcp_message_received, %{
