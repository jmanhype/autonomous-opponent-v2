defmodule AutonomousOpponentV2Core.MCP.GatewayTest do
  @moduledoc """
  Tests for the MCP Gateway supervisor and core functionality.
  """
  ...
  describe "Gateway supervisor" do
    test "starts all required child processes" do
      ...
      assert Enum.member?(child_names, AutonomousOpponentV2Core.MCP.Pool.ConnectionPool)
      assert Enum.member?(child_names, AutonomousOpponentV2Core.MCP.LoadBalancer.ConsistentHash)
      assert Enum.member?(child_names, AutonomousOpponentV2Core.MCP.Transport.Router)
    ...
    test "restarts child processes on failure" do
      ...
      |> Enum.filter(fn {name, _, _, _} -> 
        name == AutonomousOpponentV2Core.MCP.Transport.Router 
      end)
  ...
  describe "report_metrics/1" do
    test "publishes metrics to VSM S4 subsystem" do
      ...
      assert event_data.source == :mcp_gateway
