defmodule AutonomousOpponentV2Web.MCPChannelTest do
  @moduledoc """
  Tests for the MCP WebSocket channel.
  """
  
  use AutonomousOpponentV2Web.ChannelCase
  
  alias AutonomousOpponentV2Web.MCPChannel
  ...
  setup do
    ...
    {:ok, socket} = connect(AutonomousOpponentV2Web.MCPSocket, %{})
  ...
  describe "join/3" do
    test "successfully joins mcp:gateway channel", %{socket: socket} do
      assert {:ok, socket} = subscribe_and_join(socket, MCPChannel, "mcp:gateway", %{})
    ...
    test "receives welcome message on join", %{socket: socket} do
      {:ok, _socket} = subscribe_and_join(socket, MCPChannel, "mcp:gateway", %{})
    ...
    test "accepts custom client_id", %{socket: socket} do
      ...
      {:ok, socket} = subscribe_and_join(socket, MCPChannel, "mcp:gateway", %{
        "client_id" => custom_id
      })
    ...
    test "configures connection options", %{socket: socket} do
      ...
      {:ok, _socket} = subscribe_and_join(socket, MCPChannel, "mcp:gateway", params)
  ...
  describe "error scenarios" do
    test "handles WebSocket registration failure", %{socket: socket} do
      ...
      assert {:error, %{reason: "connection_failed"}} = 
        subscribe_and_join(socket, MCPChannel, "mcp:gateway")
    end
  end
end