defmodule AutonomousOpponentV2Web.MCPDashboardLiveTest do
  use AutonomousOpponentV2Web.ConnCase

  import Phoenix.LiveViewTest

  describe "MCP Dashboard" do
    test "renders dashboard", %{conn: conn} do
      {:ok, view, html} = live(conn, "/mcp/dashboard")
      
      assert html =~ "MCP Gateway Dashboard"
    ...
    test "updates metrics in real-time", %{conn: conn} do
      {:ok, view, _html} = live(conn, "/mcp/dashboard")
